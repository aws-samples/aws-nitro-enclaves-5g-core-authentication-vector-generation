...
try:
    from lte import BaseLTEAuthAlgo, FiveGRanAuthVector, Open5gsAuthVector
except ModuleNotFoundError:
    from gggpp_crypto.lte import BaseLTEAuthAlgo, FiveGRanAuthVector, Open5gsAuthVector
import logging
...
class Milenage(BaseLTEAuthAlgo):
    ...
    # Amazon addition.
    def generate_authentication_vector_open5gs(
        self, key: bytes, opc: bytes, sqn: int,
        snni: bytes, rand: bytes
    ) -> Open5gsAuthVector:
        """
        Generate the NGRAN key vector.
        Args:
            ...
            snni : bytes
                32 bit serving network name consisting of MCC and MNC
        Returns:
            Open5gsAuthVector : NamedTuple
                 Consists of (rand, xres_star, autn, kseaf)
        """
        sqn_bytes = bytearray.fromhex('{:012x}'.format(sqn))
        logging.info(f'K = {key.hex()}, OPC = {opc.hex()}, SQN = {sqn_bytes.hex()}')
        logging.info(f'rand = {rand.hex()}')
        logging.info(f'{snni}')
        ...
        logging.info(f'ck = {ck.hex()}, ik = {ik.hex()}')
        logging.debug(f'autn = {autn.hex()}')
        logging.debug(f'xres_star full = {xres_star.hex()}')
        logging.debug(f'xres_star 32:  = {xres_star.hex()[32:]}')
        logging.debug(f'kausf = {kausf.hex()}')

        return Open5gsAuthVector(rand, xres_star, autn, kausf)
    # End of Amazon addition.
